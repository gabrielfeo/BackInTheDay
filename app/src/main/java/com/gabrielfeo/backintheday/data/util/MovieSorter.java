...
public final class MovieSorter {
    ...
    private void sortByPopularityDescending() {
        movies.sort((movie1, movie2) -> {
            if (movie1.getPopularity() < movie2.getPopularity()) return 1;
            else if (movie1.getPopularity() > movie2.getPopularity()) return -1;
            else return 0;
        });
    }
    ...
    private void sortByRatingDescending() {
        movies.sort((movie1, movie2) -> {
            if (movie1.getVoteAverage() < movie2.getVoteAverage()) return 1;
            else if (movie1.getVoteAverage() > movie2.getVoteAverage()) return -1;
            else return 0;
        });
    }
